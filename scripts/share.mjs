...
const mine = changed.filter(isDemo);
...
const message =
  note || `${demos.map((d) => d.replace('src/demos/', '')).join(', ')}${who ? ` — ${who}` : ''}`;

console.log('');
try {
  // Explicit paths, never -A.
  spawnSync('git', ['add', 'src/demos', 'package.json', 'package-lock.json'], { stdio: 'inherit' });
